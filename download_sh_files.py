...
BASE_URL = "https://abdikamalov.narod.ru/sh/"
SH_DIR = "sh"
PROGRESS_FILE = "sh_download_progress.txt"
CHUNK_SIZE = 8192
...
def download_file(filename, expected_size=None):
    ...
    filepath = os.path.join(SH_DIR, filename)
...
def main():
    ...
    if not os.path.exists(SH_DIR):
        os.makedirs(SH_DIR)
        print(f"Created directory: {SH_DIR}")
    
    # Load sh_files.json
    try:
        with open('sh_files.json', 'r') as f:
            files_data = json.load(f)
    except FileNotFoundError:
        print("Error: sh_files.json not found")
        return
    except json.JSONDecodeError:
        print("Error: Invalid JSON in sh_files.json")
        return
